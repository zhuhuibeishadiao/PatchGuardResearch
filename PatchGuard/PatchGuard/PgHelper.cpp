...
unsigned char szLDEShellCode[12800] =
{
...
NTSTATUS PgHelperMapFile(wchar_t* szFileName, PVOID* lpMapped)
{
    ...
    do
    {
        if (szFileName == NULL || lpMapped == NULL)
            break;
        ...
        {
            DPRINT("open %wZ faild %p\r\n", status);
            break;
        }

        // SEC_IAMGE
        status = ZwCreateSection(&hSection, SECTION_MAP_READ, NULL, NULL, PAGE_READONLY, 0x1000000, hFile);

        if (!NT_SUCCESS(status))
        {
            DPRINT("create section faild %p\r\n", status);
            break;
        }

        status = ZwMapViewOfSection(hSection, ZwCurrentProcess(), (PVOID*)&base, 0, 0, NULL, &viewsize, ViewUnmap, 0, PAGE_READONLY);

        if (!NT_SUCCESS(status))
        {
            DPRINT("map view section faild %p\r\n", status);
            break;
        }
        ...
        {
            DPRINT("invalid pe file.\r\n");
            status = STATUS_INVALID_IMAGE_FORMAT;
            break;
        }

        for (size_t i = 0; i < viewsize - 1; i++)
        {
            if (*(PCHAR)((PCHAR)base + i) == 0xaa)
            {
                DPRINT("");
            }
        }

        DPRINT("map ntos at :%p\r\n", base);
...
{
    BOOLEAN bInitSection = FALSE;

    if (sectionName == NULL || base == NULL)
        return NULL;

    if (strlen(sectionName) == 4)
        bInitSection = true;
    ...
    {
        // INITXXXX INIT
        if (memcmp(pSection->Name, sectionName, strlen(sectionName)) == 0)
        {
            if (bInitSection)
            {
                // INITKDBG
                if (memcmp(pSection->Name, "INITK", strlen("INITK")) == 0)
                    continue;
            }
...
NTSTATUS PgHelperScanSection(IN PCCHAR sectionName,
    ...
    __in_opt PVOID* ptr)
{
    if (sectionName == NULL || base == NULL || pFeatureCode == NULL || FeatureCodeNum == 0 || ptr == NULL)
    {
        return STATUS_INVALID_PARAMETER;
    }

    auto pSection = PgHelperGetSection(sectionName, base);
    ...
    PVOID p = PgHelperGetUndocumentFunctionAddress(
        NULL, 
...
PVOID PgHelperGetSystemInformation(SYSTEM_INFORMATION_CLASS InfoClass)
{
    ...
    while (1)
    {
        if ((Info = ExAllocatePoolWithTag(NonPagedPool, Size, 'loop')) == NULL)
        {
            return NULL;
        }
...
{
    auto pBigPoolInfo = reinterpret_cast<PSYSTEM_BIGPOOL_INFORMATION>(PgHelperGetSystemInformation(SystemBigPoolInformation));

    if (pBigPoolInfo == NULL)
        return STATUS_UNSUCCESSFUL;

    if (lpPoolInfo)
    {
        *lpPoolInfo = pBigPoolInfo;
        return STATUS_SUCCESS;
    }
    ...
    for (ULONG i = 0; i < pBigPoolInfo->Count; i++) 
    {
        ...
        if (MmIsAddressValid(poolEntry.VirtualAddress))
        {
            ...
            if (!callback(bNonPaged, pp->VirtualAddress, poolEntry.SizeInBytes, poolEntry.Tag, context))
                break;
        }

    }

    ExFreePoolWithTag(pBigPoolInfo, 'loop');
...
ULONG_PTR PgHelperGetRoutineName(wchar_t* szName)
{
    ...
    ULONG_PTR ret = (ULONG_PTR)MmGetSystemRoutineAddress(&usName);
...
{
    static PVOID Lde = NULL;

    if (Lde != NULL && MmIsAddressValid(Lde))
        return Lde;

    Lde = ExAllocatePoolWithTag(NonPagedPool, sizeof(szLDEShellCode), 'edlk');

    if (Lde == NULL)
        return NULL;

    RtlCopyMemory(Lde, szLDEShellCode, sizeof(szLDEShellCode));
